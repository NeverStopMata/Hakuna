...
void TextureMgr::CreateTextureImage(const VulkanCore::VulkanContex& vk_contex,string file_path,Texture& tex) {
	...
	tex.miplevel_size = miplevel_size;
	...
	VulkanCore::CreateImage(
		...
		tex.texture_image,
		tex.texture_image_memory);
	VulkanCore::TransitionImageLayout(vk_contex, tex.texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, miplevel_size);
	VulkanCore::CopyBufferToImage(vk_contex,staging_buffer, tex.texture_image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
	VulkanCore::GenerateMipmaps(vk_contex,tex.texture_image, VK_FORMAT_R8G8B8A8_UNORM, texWidth, texHeight, miplevel_size);
	...
	tex.texture_image_view = VulkanCore::CreateImageView(vk_contex, tex.texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, miplevel_size);
	CreateTextureSampler(vk_contex, tex);
}

void TextureMgr::CreateTextureSampler(const VulkanCore::VulkanContex& vk_contex, Texture& tex) {
