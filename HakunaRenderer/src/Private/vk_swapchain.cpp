...
void VKSwapchain::CreateSurface(GLFWwindow* windows, VkInstance vk_instance_)
{
	if (glfwCreateWindowSurface(vk_instance_, windows, nullptr, &(this->surface_)) != VK_SUCCESS) {
		throw std::runtime_error("failed to create window surface!");
	}
}
void VKSwapchain::Connect(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device)
{
...
void VKSwapchain::QuerySwapChainSupport(VkPhysicalDevice physical_device, VkSurfaceKHR surface)
{
...
VkPresentModeKHR VKSwapchain::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes, bool is_vsync)
{
...
VkExtent2D VKSwapchain::ChooseSwapExtent(GLFWwindow* window, const VkSurfaceCapabilitiesKHR& capabilities)
{
...
void VKSwapchain::CreateSwapchain(GLFWwindow* window, bool vsync)
{
	...
	VulkanUtility::QueueFamilyIndices indices = VulkanUtility::FindQueueFamilies(physicalDevice_, surface_);
